...
import cromwell.pipeline.datastorage.dto.{ UUID => UserId }
      ...
      concat(
        ...
        put {
          entity(as[UserUpdateRequest]) { userUpdateRequest =>
            onComplete(userService.updateUser(UserId(accessToken.userId), userUpdateRequest)) {
        ...
        put {
          entity(as[PasswordUpdateRequest]) { passwordUpdateRequest =>
            onComplete(userService.updatePassword(UserId(accessToken.userId), passwordUpdateRequest)) {
              case Success(_)   => complete(StatusCodes.NoContent)
